...
const { extractFfmpegTar } = require('./dependencyFfmpegExtract');
const { canWriteDir } = require('./dependencyFsUtils');
...
class DependencyManager {
    constructor() {
        ...
        let appDir;
        if (app.isPackaged) {
            // In production: prefer the directory where the executable is located
            // but fall back to userData if not writable (common on Linux).
            const exeDir = path.dirname(app.getPath('exe'));
            const canWriteExeDir = canWriteDir(exeDir);
            appDir = canWriteExeDir ? exeDir : app.getPath('userData');
        } else {
        ...
        this.ytdlpPath = path.join(this.binPath, isWin ? 'yt-dlp.exe' : 'yt-dlp');
        this.ffmpegPath = path.join(this.binPath, isWin ? 'ffmpeg.exe' : 'ffmpeg');
        ...
        if (isWin) {
            ...
            this.ffmpegUrl = `https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-${arch}-gpl.tar.xz`;
        } else {
            this.ffmpegUrl = null;
        }
        // Download state
    ...
    async downloadFfmpeg(onProgress) {
        if (!this.ffmpegUrl) {
            throw new Error('Unsupported platform for ffmpeg auto-download.');
        }
        if (process.platform === 'linux' && /\.tar\.xz$/i.test(this.ffmpegUrl)) {
            return this.downloadAndExtractFfmpegTar(onProgress);
        }
    ...
    async downloadAndExtractFfmpegTar(onProgress) {
        const archivePath = path.join(this.binPath, 'ffmpeg.tar.xz');
        const extractDir = path.join(this.binPath, 'ffmpeg-extract');

        await this.downloadFile(this.ffmpegUrl, archivePath, 'ffmpeg', onProgress);
        await extractFfmpegTar({ archivePath, extractDir, ffmpegPath: this.ffmpegPath });
